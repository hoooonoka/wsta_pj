# this file contains code for training Decompositional Attention (DA) model on the SNLI dataset
import spacy
import numpy as np
...
def read_fever(path):
    ...
    f.close()
    print(count)
    return sentences1, sentences2, to_categorical(np.asarray(labels, dtype='int32'))
...
def start():
    ...
    texts,hypotheses,labels = read_fever('snli_1.0_train.jsonl')
    sem_vectors, text_vectors, hypothesis_vectors = create_dataset(nlp, texts, hypotheses, 100, 50, True)
    texts_test,hypotheses_test,labels_test = read_fever('snli_1.0_dev.jsonl')
